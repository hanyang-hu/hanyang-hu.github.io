...
2. \\(g_{\alpha\alpha} = \mathbf{I}_r$ on $U_\alpha\\).
...
It is easy to verify that \\(f: E_0 \to E_1\\) is bijective (with the inverse defined by \\(f^{-1}(v) = (\phi_\alpha^{-1}\circ \psi_\alpha)(v)\\) for all \\(v \in \pi_1^{-1}(U_\alpha)\\)) and is an \\(\mathcal{S}\\)-isomorphism (since \\(\phi_\alpha, \psi_\alpha\\) are \\(\mathcal{S}\\)-isomorphisms). 
