...
which can be interpreted as a feed-forward architecture. Similarly, we can also define parameterized Lagrangians on other manifolds (e.g., matrix Lie groups) and treat the corresponding variational integrator as a feed-forward architecture. In this sense, variational integrators and the corresponding discrete variational mechanics give us a theoretically justified principle to encode physical and geometric prior knowledge into the neural network model.
